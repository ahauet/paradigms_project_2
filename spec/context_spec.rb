...
describe Context do
  ...
  context '#activate' do
    ...
    context 'when contexts C9 C10 are created and has an adaptation on the method foo. The adaptation of context d use proceed method' do
      ...
      it 'adapt the method foo of the klaas C' do
        expect(C.new.foo).to eq 80
      end
    end
    context 'when a context adapts two methods' do
        before {
          reset_cop_state
          @c10 = Context.new
          @c10.adapt(C, :foo){1 + proceed()}
          @c10.adapt(C, :bar){1 + proceed()}
          @c10.activate
      }
      it 'adapt the method foo of the klaas C' do
        expect(C.new.bar).to eq 3
        expect(C.new.foo).to eq 2
      end
    end
    context 'when there are a lot of things' do
        before {
          reset_cop_state
          @c10 = Context.new
          @c10.adapt(C, :foo){1 + proceed()}
          ...
          @c11.adapt(C, :foo){proceed()}
          @c12 = Context.new
          @c12.adapt(C, :bar){1 + proceed()}
          @c12.adapt(C, :foo){1 + proceed()}
          @c10.activate
          @c11.activate
          @c12.activate
        }
        it 'adapt the method foo of the klaas C' do
            expect(C.new.bar).to eq 43
            expect(C.new.foo).to eq 3
        end
    end
  end
